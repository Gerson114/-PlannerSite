...
export default function Banner() {
  ...
  return (
    <>
      ...
      {/* Seção dos Cards de Serviços */}
      <div className="flex justify-center mt-[60px] lg:mt-[100px] flex-col items-center">
        <div className="max-w-[1400px] w-full px-6">
          
          {/* Grid Inteligente:
              - Mobile: Flex (Slider)
              - Tablet (md): Grid de 2 colunas
              - Notebook/PC (lg): Grid de 4 colunas
          */}
          <div 
            ref={scrollRef}
            className="
          ...
          >
            {cards.map((card, index) => (
              <a
                ...
                href={card.link}
                className="
                  group flex-shrink-0 w-[85%] md:w-full h-[320px] lg:h-[370px] 
                  rounded-3xl border border-gray-200 flex flex-col p-8 
                  transition-all duration-300 cursor-pointer 
                  hover:bg-[#432CF3] hover:border-[#432CF3] 
                  snap-center shadow-sm hover:shadow-xl
                "
              >
                <div className="bg-black self-end w-12 h-12 rounded-lg flex items-center justify-center text-white transition-colors duration-300 group-hover:bg-white group-hover:text-[#432CF3]">
        ...
        {/* Botões de Navegação: Visíveis apenas no Mobile (abaixo de 768px) */}
        <div className="flex gap-4 mt-4 md:hidden">
          <button 
            onClick={() => scroll("left")}
            className="w-12 h-12 rounded-full border border-gray-300 flex items-center justify-center active:scale-90 transition-transform"
          >
